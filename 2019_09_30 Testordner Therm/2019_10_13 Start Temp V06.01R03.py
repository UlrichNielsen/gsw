...
f6riter.writerow(["Datum", "Zeit", "Windfang", "Wz/Fenster", "Wz/Mitte", "Wz/Küche", "ArbZi", "Bad", "Schlaf", "Dh/Gart", "Dh/Mitte", "Dh/Str", "Keller"])
# und schliessen
#
f6.close ()
...
#*        print (time.strftime("%H:%M:%S") , "     " ,realSensorBezeichnung[x] , "   " , tempSensorBezeichnung[x] , " " , tempSensorWert[x] , " °C")
 ...
 # Datensatz für 6-5:54 erzeugen
    ...
    f6.write (str (lfdDatum) + ";"
                + str (lfdUhr) + ";"                      
                + str (tempSensorWert[10]) + ";"
                + str (tempSensorWert[5]) + ";"
                + str (tempSensorWert[3]) + ";"
                + str (tempSensorWert[6]) + ";"
                + str (tempSensorWert[11]) + ";"
                + str (tempSensorWert[4]) + ";"
                + str (tempSensorWert[7]) + ";"
                + str (tempSensorWert[1]) + ";"
                + str (tempSensorWert[2]) + ";"
                + str (tempSensorWert[8]) + ";"
                + str (tempSensorWert[0]) + ";" 
                + "\n")
#+       print (DLauf, "    ", time.strftime("%H:%M:%S"))
    if IlfdStd == 6:
        if IlfdMin > 0 and IlfdMin < 9:
            ...
            File6bis554 = LfdDir + LfdY + lfdM + lfdT + TagWe + TagWe + Quelle
            f6 = open(File6bis554,"w")
            f6riter = csv.writer(f6, delimiter=';', quotechar='"', quoting=csv.QUOTE_ALL)
                    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
            f6riter.writerow(["Datum", "Zeit", "Windfang", "Wz/Fenster", "Wz/Mitte", "Wz/Küche", "ArbZi", "Bad", "Schlaf", "Dh/Gart", "Dh/Mitte", "Dh/Str", "Keller"])
