...
# General Purpose I/O behandeln
    ...
    global LfdDir, TagWe, QuelleGSW, QuelleT
    global Jahr, lfdJahr
    global TdJ, lfdTdJ
    global MdJ, lfdMdJ
    global TdM, lfdTdM
    ...
    global Filename
    global FileWoWe
    global File6bis554, Gile6bis554 
    global Flag_Anf6            # Flag für 6:00 Anfang
    global d, f, g, l, s, f6, g6	# fileidentifier
...
#   Zeiten und Intervallwerte initialisieren
   ...
   ## d.close ()
    ...
    File6bis554 = LfdDir + LfdY + lfdM + lfdT + TagWe + QuelleGSW
    f6 = open(File6bis554,"w")
    f6riter = csv.writer(f6, delimiter=';', quotechar='"', quoting=csv.QUOTE_ALL)
    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
    f6riter.writerow(["Datum", "Zeit", "Gas_6m", "Strom_6m_diff","Wasser_6m", "Vorlauf", "Aussen"])

    # und schliessen
    #
    f6.close ()

    Gile6bis554 = LfdDir + LfdY + lfdM + lfdT + TagWe + QuelleT
    g6 = open(Gile6bis554,"w")
    g6riter = csv.writer(g6, delimiter=';', quotechar='"', quoting=csv.QUOTE_ALL)
    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
    g6riter.writerow(["Datum", "Zeit", "Windfang", "Wz/Fenster", "Wz/Mitte", "Wz/Kueche", "ArbZi", "Bad", "Schlaf", "Dh/Gart", "Dh/Mitte", "Dh/Str", "Vorlauf", "Aussen"])
...
# Zeiten setzen
    ...
    while zeitAktuell < zeitEnde: 
       ...
       ## d = open(Filename,"a+")
        ...
        # Öffnen csv-Quelle für Stromzähler
        ...
        # einfügen in DB 
                ...
                if Strom_6m > 0:
                    ...
                    # print(time.strftime("%H:%M:%S"),Strom_6m_diff, Strom_6m_vor)
                    if Strom_6m_diff > 5999:     # erster Wert vor Diff-Bildung
                        # print(time.strftime("%H:%M:%S"),Strom_6m_diff)
                        Strom_6m_diff = 0
                        # print(time.strftime("%H:%M:%S"),Strom_6m_diff) 
        ...
        if lfdStd != Std:       # Volle Stunde
            ...
            if lfdStd == 6:
                if lfdMin < 10:
                    ...
                    f6riter = csv.writer(f6, delimiter=';', quotechar='"', quoting=csv.QUOTE_ALL)
                    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
                    f6riter.writerow(["Datum", "Zeit", "Gas_6m", "Strom_6m_dif", "Cnt_W_6m", "Vorlauf", "Aussen"])
...
# Quit on Ctrl-c
    ...
    f6.close ()
    g6.close ()
    print(time.strftime("%H:%M:%S"), "Files geschlossen") 
    # Alle ausschalten
    GPIO.output(channel_out_r, GPIO.LOW)       # Rot aus (Gas)
    GPIO.output(channel_out_ge, GPIO.LOW)      # Gelb aus (Wasser)
    GPIO.output(channel_out_b, GPIO.LOW)       # Blau aus (Wasser)    
    GPIO.output(channel_out_gr, GPIO.LOW)      # Grün aus (Gas)
    GPIO.output(channel_out_we, GPIO.LOW)       # Weiss aus (Lebenszeichen)
    GPIO.output(channel_out_ro, GPIO.LOW)       # Rot aus (Temperatur)
    GPIO.output(channel_out_ws, GPIO.LOW)       # Weiss aus (Temperatur)
# Cleanup GPIO
finally:
    GPIO.cleanup() 
