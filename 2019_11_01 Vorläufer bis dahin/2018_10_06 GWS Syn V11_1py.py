...
#*********************************************************************
#
# Dateien erzeugen bzw. öffnen in eigenem Folder derzeit nicht auf NAS
# ausgeblendet mit #DS
# Daten-Struktur 
...
#   Zeiten und Intervallwerte initialisieren
    ...
    path="/home/pi/Lokale Erg/"
    os.chdir(path) 

    # Öffnen eines "neuen" Files (Löschen alter Inhalte und Überschriften schreiben)
   ...
   ## d.close ()
    ...
    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
    f6riter.writerow(["Datum", "Zeit", "Gas_6m", "Strom_6m_diff","Wasser_6m", "Strom_6m", "Voll"])
        ...
        if lfdStd != Std:       # Volle Stunde
            ...
            if lfdStd == 6:
                if lfdMin < 10:
                    ...
                    f6riter = csv.writer(f6, delimiter=';', quotechar='"', quoting=csv.QUOTE_ALL)
                    # Schreiben der Liste als CSV-Datensatz ergibt "Überschrift"
                    f6riter.writerow(["Datum", "Zeit", "Gas_6m", "Strom_6m_dif", "Cnt_W_6m", "Strom_6m", "Voll"])
